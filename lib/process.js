...
module.exports = async (proccesingConfig, tmpDir, log) => {
  ...
  // await fs.writeSync(writeStream, csvStringify(fields, { delimiter: ';' }))

  const files = await fs.readdir(tmpDir)
  for (const file in files) {
    const pathToFile = path.join(tmpDir, files[file])
    if (fs.lstatSync(pathToFile).isDirectory()) {
      log.info('Traitement de l\'archive ' + files[file])
      ...
      for (const csvFile in csvFiles) {
        log.info('Traitement du fichier ' + csvFiles[csvFile])
        ...
        if (source.includes('waldec')) {
          for (const line of lines) {
            ...
            let departement = line[22].toString()
            if (departement.slice(0, 2) === '97') {
              departement = '97'
            } else {
              departement = departement.slice(0, 2)
            }
            ...
            const item = [
              ...
              line[3],
              line[4],
              dateCreation ? `${dateCreation.getDate().toString().padStart(2, 0)}/${dateCreation.getMonth().toString().padStart(2, 0) + 1}/${dateCreation.getFullYear()}` : '',
              dateDeclaration ? `${dateDeclaration.getDate().toString().padStart(2, 0)}/${dateDeclaration.getMonth().toString().padStart(2, 0) + 1}/${dateDeclaration.getFullYear()}` : '',
              datePublication ? `${datePublication.getDate().toString().padStart(2, 0)}/${datePublication.getMonth().toString().padStart(2, 0) + 1}/${datePublication.getFullYear()}` : '',
              dateDissolution ? `${dateDissolution.getDate().toString().padStart(2, 0)}/${dateDissolution.getMonth().toString().padStart(2, 0) + 1}/${dateDissolution.getFullYear()}` : '',
              line[9],
              line[10],
              line[11],
              line[12],
              ...
              line[36],
              line[37],
              `${dateMAJ.getDate().toString().padStart(2, 0)}/${dateMAJ.getMonth().toString().padStart(2, 0) + 1}/${dateMAJ.getFullYear()} à ${dateMAJ.getHours().toString().padStart(2, 0)}h${dateMAJ.getMinutes().toString().padStart(2, 0)}`,
              departement,
              source
            ]
            items.push(item)
          ...
          for (const line of lines) {
            ...
            let departement = line[17].toString()
            if (departement.slice(0, 2) === '97') {
              departement = '97'
            } else {
              departement = departement.slice(0, 2)
