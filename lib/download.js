...
module.exports = async (processingConfig, tmpDir, axios, log) => {
  ...
  const json = await axios.get(url + processingConfig.processFile + '/')
  const filtre = []
  const data = json.data.resources
  const date = `${month[new Date().getMonth()]} ${new Date().getFullYear()}`
  log.info(`Mois en cours : ${date}, Recherche des données correspondantes`)
  for (let i = 0; i < data.length; i++) {
    if (data[i].title.includes(`${date}`)) {
      filtre.push(data[i].url)
    }
  }
  if (filtre.length === 0) {
    await log.warning('Aucun fichier trouvé, récupération des données du dernier mois')
    filtre.push(data[0].url)
    filtre.push(data[1].url)
    await log.info('Dernier fichier en date : ' + data[0].title)
  }
